...
bool SSCB_wrSensorReg16_8(uint16_t address, uint16_t regID, uint16_t regDat)
{
	...
	if(HAL_I2C_Master_Transmit(&hi2c1, address,
...
int SSCB_wrSensorRegs16_8(const struct sensor_reg reglist[])
{
  ...
  while ((reg_addr != 0xffff) | (reg_val != 0xff))
  {
    ...
    err = SSCB_wrSensorReg16_8(OV2640_CHIP_SSCB_BUS_ADDR_W, reg_addr, reg_val);
    next++;
  }
  return err;
}
...
bool SSCB_rdSensorReg16_8(uint16_t address, uint16_t regID, uint8_t* regDat)
{
	...
	if(HAL_I2C_Master_Transmit(&hi2c1, rw_address, (uint8_t*)tx_buffer, sizeof(tx_buffer), SSCB_TIMEOUT) != HAL_OK)
		return false;

	rw_address = rw_address + 1;

	if(HAL_I2C_Master_Receive(&hi2c1, rw_address, (uint8_t*)regDat, sizeof(tx_buffer) - 1,  SSCB_TIMEOUT) != HAL_OK)
	  return false;
